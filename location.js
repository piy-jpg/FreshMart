...
(function() {
  ...
  /**
   * -------------------------------------------------------------
   * GOOGLE MAPS PLATFORM DYNAMIC LOADER
   * -------------------------------------------------------------
   */
  ...
  function loadGoogleMapsScript() {
    ...
    googleMapsLoadingPromise = new Promise(async (resolve) => {
      ...
      // If no valid key provided in .env, don't break the app; enable interactive fallback
      if (!key || key.length < 6) {
      ...
      script.onerror = () => {
        console.warn('Google Maps JS SDK could not be loaded. Operating in interactive fallback mode.');
        googleMapsLoaded = false;
        resolve(false);
      };
  ...
  /**
   * -------------------------------------------------------------
   * BLINKIT-STYLE LOCATION CONTROLLER STATE & MODAL DOM
   * -------------------------------------------------------------
   */
  ...
  // Fallback map state when Google Maps API key is not active
      ...
      >
        ...
        <div id="location-step-1-view" class="flex-1 flex flex-col min-h-0 overflow-y-auto">
          ...
          <div class="p-3.5 pb-2 bg-white space-y-2.5 shrink-0 z-10 border-b border-stone-100">
            ...
            <!-- Places Autocomplete Search Bar (FEATURE 4) -->
            ...
            <!-- Quick Action: Use My Current Location Button (FEATURE 3) -->
            ...
            <!-- Missing Google Maps API Key Notice (Clean Informational) -->
            <div id="location-maps-key-notice" class="hidden p-2.5 rounded-xl bg-emerald-50/70 border border-emerald-200 text-emerald-950 text-xs flex items-start gap-2">
              <i data-lucide="info" class="w-4 h-4 text-emerald-700 shrink-0 mt-0.5"></i>
              <div class="flex-1 text-[11px] leading-relaxed text-stone-600">
                <strong class="text-emerald-950 font-bold">Interactive Fallback Mode:</strong> Set <code class="px-1 py-0.5 bg-white rounded border border-emerald-200 font-mono text-[10px]">GOOGLE_MAPS_API_KEY</code> in your <code class="px-1 py-0.5 bg-white rounded border border-emerald-200 font-mono text-[10px]">.env</code> file for live Google Maps tiles. The interactive pin and geocoding work fully below.
              </div>
            </div>
          </div>
          ...
          <!-- ======================================================== -->
          <!-- FEATURE 5: INTERACTIVE MAP CONTAINER WITH FIXED PIN      -->
          <!-- ======================================================== -->
          <div class="relative w-full h-64 sm:h-72 bg-stone-100 overflow-hidden shrink-0 border-b border-stone-200 select-none">
            ...
            <!-- Fallback Canvas Map (Rendered when Google Maps Key is not provided) -->
            <div id="sabjihub-fallback-map" class="hidden absolute inset-0 bg-stone-200 flex flex-col items-center justify-center cursor-grab active:cursor-grabbing">
              ...
              <div class="absolute bottom-2 left-2 pointer-events-none bg-black/60 backdrop-blur-xs text-white text-[10px] font-mono px-2 py-0.5 rounded-md">
                Drag to pan • Center coordinates: <span id="fallback-coords-display">28.5580, 77.3320</span>
              </div>
            </div>

            <!-- FIXED PIN IN THE CENTER (Blinkit Style) -->
            <div 
            ...
            <div class="absolute top-2 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
              <span class="px-2.5 py-0.5 rounded-full bg-white/90 backdrop-blur-xs text-stone-600 text-[10px] font-bold shadow-xs border border-stone-200">
                Move pin to exact delivery point
              </span>
            </div>
          </div>
          ...
          <div class="p-4 bg-white space-y-3 flex-1 flex flex-col justify-between">
            <div class="space-y-2">
              ...
              <!-- FEATURE 6: Serviceability / Delivery Availability Badge -->
            ...
            <!-- Action Button: Confirm & Continue (Step 1 -> Step 2) -->
            <div class="pt-1">
              <button 
                type="button" 
        ...
        <!-- ======================================================== -->
        <!-- STEP 2: ADD DELIVERY DETAILS FORM (FEATURE 7)            -->
        <!-- ======================================================== -->
        <div id="location-step-2-view" class="hidden flex-1 flex flex-col min-h-0 overflow-y-auto p-5 space-y-4">
          ...
          <form id="location-details-form" onsubmit="window.sabjihubLocation.saveDeliveryDetails(event)" class="space-y-3.5 flex-1">
            ...
            <!-- Save Address As: Home / Work / Other (Pills) -->
            <div>
            ...
            <!-- Submit Button: Save Address (FEATURE 7, 8) -->
            <div class="pt-3">
              <button 
                type="submit" 
  ...
  /**
   * -------------------------------------------------------------
   * FALLBACK CANVAS MAP CONTROLLER (When Google Maps key is omitted)
   * Simulates visual map tiles and supports drag & zoom
   * -------------------------------------------------------------
   */
  function initFallbackMapDragging() {
    ...
    // Mouse / Touch Dragging on Fallback Map
    ...
    window.addEventListener('mousemove', (e) => {
      ...
      // Coordinate shift proportional to zoom
      ...
      // Trigger map idle reverse geocoding
    ...
    // Touch events for mobile fallback dragging
  ...
  /**
   * -------------------------------------------------------------
   * MAP CONTROLLER: GOOGLE MAPS INSTANTIATION & IDLE LISTENER
   * -------------------------------------------------------------
   */
  async function initOrUpdateMap(lat, lng) {
    ...
    const noticeEl = document.getElementById('location-maps-key-notice');
    if (!mapDiv) return;
    ...
    if (isGoogleMapsReady && window.google && window.google.maps) {
      ...
      if (!currentMapInstance) {
        ...
        // FEATURE 5: Listen to map 'idle' event (Fixed center pin reading)
      ...
      // Fallback Mode (API key missing or blocked)
  ...
  /**
   * FEATURE 5: Debounced reverse geocoding on map idle
   */
  function handleMapIdle(lat, lng) {
    clearTimeout(mapIdleDebounceTimer);

    // Show quick scanning state in preview
    ...
    mapIdleDebounceTimer = setTimeout(async () => {
      try {
        ...
        // FEATURE 6: Delivery Availability Check
  ...
  /**
   * FEATURE 6: Update Delivery Availability UI
   */
  function updateAvailabilityBadge(avail) {
    const badge = document.getElementById('location-availability-badge');
    const textEl = document.getElementById('location-availability-text');
    const etaEl = document.getElementById('location-availability-eta');
    const confirmBtn = document.getElementById('location-confirm-continue-btn');
    if (!badge || !textEl || !etaEl || !confirmBtn) return;
    ...
    if (avail.serviceable) {
      ...
      confirmBtn.innerHTML = `<span>Confirm & Continue</span><i data-lucide="arrow-right" class="w-4 h-4"></i>`;
      confirmBtn.className = 'w-full py-3 px-4 rounded-2xl bg-emerald-800 hover:bg-emerald-900 text-white font-bold text-sm shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer';
    } else {
      ...
      confirmBtn.innerHTML = `<span>Change Location</span><i data-lucide="map-pin" class="w-4 h-4"></i>`;
      confirmBtn.className = 'w-full py-3 px-4 rounded-2xl bg-stone-200 text-stone-500 font-bold text-sm transition-all flex items-center justify-center gap-2 cursor-not-allowed opacity-75';
    }
  ...
  /**
   * FEATURE 9: Render Saved Addresses List
   */
  ...
  window.sabjihubLocation = {
    ...
    /**
     * FEATURE 1 & 2: Open Location Selection Modal
     */
    async openModal() {
    ...
    closeModal() {
      ...
      // Close legacy modal if present
      const legacyModal = document.getElementById('location-modal');
      if (legacyModal) {
    ...
    /**
     * FEATURE 7 & 8: Save Address Details & Update Navbar
     */
    ...
    /**
     * FEATURE 3: Use My Current Location (GPS)
     */
    async detectCurrentLocation() {
      ...
      try {
        ...
        if (err.code === 1 || err.code === 'PERMISSION_DENIED') {
          isDenied = true;
          msg = 'Location permission is required to automatically detect your location.';
          ...
          msg = err.message || 'Unable to access your location. Please search manually.';
        }
    ...
    /**
     * FEATURE 4: Places Autocomplete Search Handler
     */
    ...
    async selectSearchResult(idx) {
      ...
      // Resolve coordinates if missing (e.g. from Google Places place_id)
    ...
    /**
     * FEATURE 9: Select Saved Address (1-Click selection)
     */
