...
(function() {
  ...
  async function syncGuestCartWithAccount() {
    try {
      const localCart = JSON.parse(localStorage.getItem('sabjihub_cart') || '{}');
      ...
      if (res.ok) {
        const data = await res.json();
        if (data.cart) {
          localStorage.setItem('sabjihub_cart', JSON.stringify(data.cart));
  ...
  window.reorderCustomerOrder = async function(orderId) {
    try {
      ...
      if (res.ok) {
        ...
        const currentCart = JSON.parse(localStorage.getItem('sabjihub_cart') || '{}');
        ...
        localStorage.setItem('sabjihub_cart', JSON.stringify(currentCart));
        if (typeof window.renderCartDrawerItems === 'function') {
          window.renderCartDrawerItems();
        }
        closeAccountDashboard();
      ...
      <div id="auth-modal" class="fixed inset-0 z-50 bg-stone-900/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 hidden">
        <div class="bg-[#fdfcf7] rounded-3xl max-w-md w-full p-6 sm:p-7 shadow-2xl border border-stone-200/80 relative max-h-[94vh] overflow-y-auto">
          ...
          <div class="text-center mb-5">
            ...
            <h3 class="text-lg sm:text-xl font-heading font-black text-emerald-950">Welcome to Sabji<span class="text-emerald-600">Hub</span></h3>
          ...
          <div id="auth-view-signin" class="space-y-4">
            ...
            <form onsubmit="submitSignIn(event)" class="space-y-3">
              <div>
                ...
                <input id="signin-email" type="email" required placeholder="you@example.com" class="w-full px-3.5 py-2.5 rounded-2xl border border-stone-200 bg-white text-xs font-medium text-stone-900 focus:outline-none focus:border-emerald-600 focus:ring-4 focus:ring-emerald-500/10 transition-all" />
              </div>
