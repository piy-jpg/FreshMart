...
function extractSessionCookie(setCookies) {
  for (const c of setCookies) {
    if (c.startsWith('sjh_session=') || c.startsWith('freshmart_session=')) {
      return c.split(';')[0];
    }
  }
  return '';
...
async function runTests() {
  ...
  try {
    ...
    let ownerLogin = await makeRequest({ path: '/api/auth/login', method: 'POST' }, {
      identifier: 'piyushverma730929@gmail.com',
      password: 'password123'
    });
    if (ownerLogin.statusCode !== 200) {
      ownerLogin = await makeRequest({ path: '/api/auth/login', method: 'POST' }, {
        identifier: 'owner@freshmart.local',
        password: 'password123'
      });
    }
    ...
    // Step 4: Owner Advances Order and Assigns / Hands Over to Pappu
    await makeRequest({ path: `/api/owner/orders/${testOrderId}/status`, method: 'PATCH' }, { status: 'CONFIRMED' }, ownerCookie);
    await makeRequest({ path: `/api/owner/orders/${testOrderId}/status`, method: 'PATCH' }, { status: 'PICKING' }, ownerCookie);
    await makeRequest({ path: `/api/owner/orders/${testOrderId}/status`, method: 'PATCH' }, { status: 'PACKING' }, ownerCookie);
    await makeRequest({ path: `/api/owner/orders/${testOrderId}/status`, method: 'PATCH' }, { status: 'READY_FOR_HANDOVER' }, ownerCookie);
    ...
    // Transition 8e: Verify Customer OTP
    const currentOrderData = await makeRequest({ path: `/api/orders/${testOrderId}`, method: 'GET' }, null, pappuCookie);
    const otp = currentOrderData.body.deliveryOtp || currentOrderData.body.otp;
    await makeRequest({ path: `/api/delivery/orders/${testOrderId}/verify-otp`, method: 'POST' }, { otp }, pappuCookie);

    // Transition 8f: ARRIVED -> DELIVERED (with COD cash collection)
    const deliverRes = await makeRequest({ path: `/api/delivery/orders/${testOrderId}/deliver`, method: 'POST' }, {
      cashCollected: true,
      otp
    }, pappuCookie);
