...
// Master Vegetable Catalog
let allVegetablesData = [
  {
    id: 'tomato',
    name: 'Fresh Tomato',
...
// Global Application State (User-Isolated Cart)
let cart = {};

function getCurrentUserId() {
  if (window.sabjihubAuth && window.sabjihubAuth.user && window.sabjihubAuth.user.id) {
    return window.sabjihubAuth.user.id;
  }
  return null;
}
...
function getCartStorageKey(userId) {
  ...
  return uid ? `freshmart_cart_${uid}` : 'freshmart_cart_guest';
}

window.loadUserCart = async function(userId = null) {
  const uid = userId || getCurrentUserId();
  if (uid) {
    try {
      ...
      if (res.ok) {
        ...
        try { localStorage.setItem(getCartStorageKey(uid), JSON.stringify(cart)); } catch (e) {}
        updateCartUI();
        if (typeof applyFiltersAndRender === 'function' && document.getElementById('veg-products-grid')) applyFiltersAndRender();
        if (typeof renderHomeProductGrid === 'function' && document.getElementById('products-grid')) renderHomeProductGrid();
        return cart;
      }
    } catch (e) {}
    cart = loadStoredState(getCartStorageKey(uid), {});
  } else {
    cart = loadStoredState('freshmart_cart_guest', {});
  }
  updateCartUI();
  if (typeof applyFiltersAndRender === 'function' && document.getElementById('veg-products-grid')) applyFiltersAndRender();
  if (typeof renderHomeProductGrid === 'function' && document.getElementById('products-grid')) renderHomeProductGrid();
...
window.saveCartState = function(items = cart) {
  const uid = getCurrentUserId();
  const key = getCartStorageKey(uid);
  try {
    localStorage.setItem(key, JSON.stringify(items));
    // Clean up legacy global cart keys to prevent leakage across users
    localStorage.removeItem('sabjihub_cart');
...
window.resetCartState = function() {
  ...
  try {
    ...
    localStorage.removeItem('sabjihub_cart');
    localStorage.removeItem('freshmart_cart');
    localStorage.removeItem('sabjihub_coupon');
    const uid = getCurrentUserId();
    if (uid) {
      localStorage.removeItem(`freshmart_cart_${uid}`);
    }
  } catch (e) {}
  updateCartUI();
  if (typeof applyFiltersAndRender === 'function' && document.getElementById('veg-products-grid')) applyFiltersAndRender();
  if (typeof applyFruitFiltersAndRender === 'function' && document.getElementById('fruit-products-grid')) applyFruitFiltersAndRender();
  if (typeof applyGroceryFiltersAndRender === 'function' && document.getElementById('grocery-products-grid')) applyGroceryFiltersAndRender();
  if (typeof applyOffersFiltersAndRender === 'function' && document.getElementById('offers-products-grid')) applyOffersFiltersAndRender();
  if (typeof renderHomeProductGrid === 'function' && document.getElementById('products-grid')) renderHomeProductGrid();
};
...
// Unified Product Lookup Helper (Across all products & Combos)
function findAnyProduct(productId) {
  if (!productId) return null;
  const cleanId = String(productId).trim();
  const all = getAllCatalogProducts();
  return (
    all.find(p => p.id === cleanId || p.dbId === cleanId || p.storefrontId === cleanId) ||
    all.find(p => p.id === 'prod_' + cleanId || p.dbId === 'prod_' + cleanId) ||
    all.find(p => ('prod_' + p.id) === cleanId) ||
    all.find(p => p.id.replace(/^prod_/, '') === cleanId.replace(/^prod_/, '')) ||
    all.find(p => p.sku === cleanId) ||
    all.find(p => p.name && p.name.toLowerCase() === cleanId.toLowerCase()) ||
    null
  );
}
...
// Helper to map a backend DB product object into the normalized storefront schema
function mapDbProductToStorefront(p) {
  if (!p) return null;
  const cleanId = (p.storefrontId || p.id || '').replace(/^prod_/, '');
  const id = p.id || ('prod_' + cleanId);
  const effectivePrice = Number(p.price !== undefined ? p.price : (p.sellingPrice || 0));
  const effectiveMrp = Number(p.mrp !== undefined ? p.mrp : (p.originalPrice || Math.round(effectivePrice * 1.25)));
  ...
  const inStock = stock > 0 && !isSuspended && p.status !== 'OUT_OF_STOCK';
  ...
  if (Array.isArray(p.weights) && p.weights.length > 0) {
    ...
    weights = [{
      ...
      discount: effectiveMrp > effectivePrice ? `${Math.round(((effectiveMrp - effectivePrice) / effectiveMrp) * 100)}% OFF` : 'Best Value',
      savings: effectiveMrp > effectivePrice ? (effectiveMrp - effectivePrice) : 0
    }];
  }

  const cat = (p.category || '').toLowerCase();
  let defaultCategories = ['all'];
  if (Array.isArray(p.categories) && p.categories.length > 0) {
    defaultCategories = [...p.categories];
  } else {
    if (cat.includes('fruit')) defaultCategories.push('fruits', 'seasonal', 'tropical', 'fresh');
    else if (cat.includes('groc') || cat.includes('pant')) defaultCategories.push('grocery', 'staples', 'daily');
    else defaultCategories.push('vegetables', 'organic', 'daily', 'root', 'leafy');
    if (p.subcategory) defaultCategories.push(p.subcategory.toLowerCase().replace(/[^a-z0-9]/g, '-'));
  }
  if (!defaultCategories.includes('all')) defaultCategories.unshift('all');

  return {
    id: cleanId,
    dbId: id,
    storefrontId: p.storefrontId || cleanId,
    name: p.name || p.title || 'Fresh Item',
    hindiName: p.hindiName || '',
    category: p.category || 'Vegetables',
    catalogType: cat.includes('fruit') ? 'fruits' : (cat.includes('groc') || cat.includes('pant') ? 'grocery' : 'vegetables'),
    subcategory: p.subcategory || 'Daily Fresh',
    categories: defaultCategories,
    rating: p.rating || 4.8,
    reviewsCount: p.reviewsCount || 120,
    badge: p.badge || (p.discountPercent > 20 ? `${p.discountPercent}% OFF` : (stock > 0 && stock <= 15 ? 'Low Stock' : 'Farm Fresh')),
    badgeType: p.badgeType || (stock > 0 && stock <= 15 ? 'warning' : 'bestseller'),
    inStock: inStock,
    stock: stock,
    stockCount: stock,
    status: p.status || (stock > 0 ? 'ACTIVE' : 'OUT_OF_STOCK'),
    image: p.image || 'https://images.unsplash.com/photo-1540420773420-3366772f4999?auto=format&fit=crop&w=400&q=80',
    gallery: p.gallery || [
      { url: p.image || 'https://images.unsplash.com/photo-1540420773420-3366772f4999?auto=format&fit=crop&w=400&q=80', title: (p.name || 'Produce') + ' - Fresh' }
    ],
    description: p.description || `${p.name} - 100% farm-fresh direct harvest, quality checked and delivered in 90 minutes.`,
    origin: p.farmer || p.origin || 'Karnataka Organic Kisan Network',
    weights: weights,
    selectedWeightIndex: 0,
    price: effectivePrice,
...
function applyStoreStatusToUI(statusData) {
  ...
  // 1. Manage persistent premium animated top banner across pages
  ...
  if (isOffline) {
    ...
    if (!banner) {
      ...
      banner.className = 'offline-banner-wrapper banner-enter';
    ...
    banner.innerHTML = `
      <div class="relative py-2.5 sm:py-3 px-3 sm:px-6 lg:px-8">
        <!-- Ambient floating background glow lights -->
        <div class="offline-ambient-orb-1"></div>
        <div class="offline-ambient-orb-2"></div>
        <!-- Horizontal Shimmer Light Streak -->
        <div class="offline-shimmer-streak"></div>

        <div class="max-w-7xl mx-auto flex items-center justify-between gap-3 text-xs sm:text-sm relative z-10">
          <div class="flex items-center gap-2.5 sm:gap-3.5 flex-wrap sm:flex-nowrap">
            <!-- Pulsing Status Dot & Primary Label -->
            <div class="flex items-center gap-2 shrink-0">
              <span class="offline-pulse-dot" aria-hidden="true"></span>
              <span class="font-heading font-black tracking-wider text-white uppercase text-xs sm:text-sm drop-shadow-xs">
                STORE TEMPORARILY OFFLINE
              </span>
            </div>

            <!-- Divider -->
            <span class="text-rose-400/40 hidden sm:inline text-xs font-light" aria-hidden="true">•</span>

            <!-- Subtitle Notice Message -->
            <span class="text-rose-100/90 text-xs sm:text-xs font-medium leading-tight">
              ${msg}
            </span>
          </div>

          <!-- Premium Right-Side Glass Pill -->
          <div class="shrink-0 hidden sm:flex items-center">
            <div class="offline-glass-pill" role="status">
              <span class="w-1.5 h-1.5 rounded-full bg-rose-400 animate-ping" aria-hidden="true"></span>
              <span>ORDERS PAUSED</span>
            </div>
          </div>
        </div>

        <!-- Mobile pill below on compact screens -->
        <div class="sm:hidden flex items-center justify-between pt-1.5 mt-1 border-t border-rose-500/20 text-[10px] text-rose-300 relative z-10">
          <span class="font-medium">Ordering will resume shortly</span>
          <div class="offline-glass-pill text-[9px] px-2 py-0.5">
    ...
    if (banner && !banner.classList.contains('hidden')) {
      ...
      setTimeout(() => {
        if (currentStoreStatus.status === 'LIVE' && banner) {
...
window.renderTrackOrderModalContent = async function(orderId) {
  ...
  if (!order) {
    order = {
      ...
      deliveryBoyName: 'Ramesh K. (EV Pilot #42)',
  ...
  modalBox.innerHTML = `
    ...
    <div class="flex items-center justify-between pb-4 border-b border-stone-100 mb-5">
      <div class="flex items-center gap-2.5">
        <div class="w-10 h-10 rounded-2xl bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold text-lg">
          ${isDelivered ? '🎉' : isCancelled ? '❌' : isFailed ? '⚠️' : '🚚'}
        </div>
        <div>
          <div class="flex items-center gap-2">
            <h3 class="font-bold text-base sm:text-lg text-emerald-950">
              ${isDelivered ? 'Order Delivered!' : isCancelled ? 'Order Cancelled' : isFailed ? 'Delivery Attempt Failed' : 'Live Order Tracking'}
            </h3>
            ${!isDelivered && !isCancelled && !isFailed ? '<span class="w-2 h-2 rounded-full bg-emerald-500 animate-ping"></span>' : ''}
          </div>
          <p class="text-xs text-stone-500">Order ID: <strong class="font-mono text-emerald-900">#${order.orderId || order.id}</strong> • ${order.items?.length || 1} Items</p>
        </div>
      </div>
    ...
    ` : isCancelled ? `
      <!-- CANCELLED CARD -->
      <div class="p-5 rounded-3xl bg-rose-50 border border-rose-200 text-center mb-6 space-y-3">
        <div class="w-12 h-12 mx-auto rounded-2xl bg-rose-600 text-white flex items-center justify-center text-xl shadow-md">✕</div>
        <div>
          <h4 class="font-bold text-base text-rose-950">This order has been cancelled</h4>
          <p class="text-xs text-rose-700 mt-1">${order.cancellationReason || 'Produce restocked to hub inventory.'}</p>
        </div>
      </div>
    ` : `
      ...
      <div class="p-4 rounded-2xl bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-300 mb-5 flex items-center justify-between shadow-2xs">
        <div>
          ...
          <span class="text-3xl font-mono font-black text-emerald-950 tracking-[0.2em]">${order.deliveryOtp || '----'}</span>
        </div>
      ...
      <div class="p-4 rounded-2xl bg-gradient-to-r from-emerald-900 to-emerald-800 text-white mb-5 shadow-md">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="text-xs font-mono bg-white/10 px-2 py-0.5 rounded-full font-bold">⚡ ${order.deliverySlot || 'Express 90-Min'}</span>
        </div>
        <div class="flex items-baseline gap-2">
          <span class="text-3xl sm:text-4xl font-black text-white">${order.estimatedDeliveryTime || (currentStatus === 'ARRIVED' ? 'Arrived!' : '28 Mins')}</span>
          <span class="text-xs text-emerald-200">${currentStatus === 'ARRIVED' ? 'At Doorstep' : 'Remaining'}</span>
        </div>
        <div class="w-full h-2 bg-emerald-950/60 rounded-full overflow-hidden mt-3">
          <div class="h-full bg-gradient-to-r from-emerald-400 to-amber-300 ${getProgressWidth(currentStatus)} rounded-full transition-all duration-500"></div>
        </div>
      </div>
      ...
      <div class="p-3.5 rounded-2xl bg-stone-50 border border-stone-200 mb-5 relative overflow-hidden">
        <div class="flex items-center justify-between text-xs mb-2">
          ...
          <span class="text-[10px] text-emerald-700 font-bold bg-emerald-100 px-2 py-0.5 rounded-full">
            ${currentStatus === 'ARRIVED' ? 'Arrived at Doorstep' : ['OUT_FOR_DELIVERY', 'PICKED_UP'].includes(currentStatus) ? 'EV Pilot on Live Route' : 'Preparing at Hub'}
          </span>
        </div>
        <svg class="w-full h-16" viewBox="0 0 400 60" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <circle cx="${currentStatus === 'ARRIVED' ? 360 : ['OUT_FOR_DELIVERY', 'PICKED_UP'].includes(currentStatus) ? 200 : 40}" cy="30" r="14" fill="#10b981" class="animate-pulse" />
          <text x="${currentStatus === 'ARRIVED' ? 354 : ['OUT_FOR_DELIVERY', 'PICKED_UP'].includes(currentStatus) ? 194 : 34}" y="34" fill="white" font-size="12">🛵</text>
          <!-- Customer Pin -->
        ...
        <div class="flex justify-between text-[10px] text-stone-500 font-semibold px-1">
          ...
          <span class="text-emerald-800 font-bold">${currentStatus === 'ARRIVED' ? 'Arrived' : 'In Fulfillment'}</span>
          <span>${order.deliveryAddress?.city || 'Indiranagar'}</span>
        </div>
      </div>

      <!-- Live Lifecycle Milestones -->
      <div class="space-y-3 mb-5 relative pl-2">
        <div class="absolute left-6 top-3 bottom-3 w-0.5 bg-emerald-200"></div>

        ${renderLifecycleStep('ORDER_RECEIVED', 'Order Received', 'Your order was received and queued for fulfillment.', currentStatus, ['ORDER_RECEIVED', 'ORDER_PLACED', 'ORDER_CONFIRMED', 'CONFIRMED', 'PICKING', 'PACKING', 'READY_FOR_HANDOVER', 'READY_FOR_PICKUP', 'HANDED_TO_DELIVERY_BOY', 'ASSIGNED', 'DELIVERY_BOY_ACCEPTED', 'ACCEPTED', 'PICKED_UP', 'OUT_FOR_DELIVERY', 'ARRIVED', 'CUSTOMER_VERIFIED', 'DELIVERED'])}
        ${renderLifecycleStep('ORDER_CONFIRMED', 'Order Confirmed', 'Verified & accepted by Fresh Mart store manager.', currentStatus, ['ORDER_CONFIRMED', 'CONFIRMED', 'PICKING', 'PACKING', 'READY_FOR_HANDOVER', 'READY_FOR_PICKUP', 'HANDED_TO_DELIVERY_BOY', 'ASSIGNED', 'DELIVERY_BOY_ACCEPTED', 'ACCEPTED', 'PICKED_UP', 'OUT_FOR_DELIVERY', 'ARRIVED', 'CUSTOMER_VERIFIED', 'DELIVERED'])}
        ${renderLifecycleStep('PACKING', 'Produce Harvested & Eco-Packed', currentStatus === 'PICKING' ? 'Warehouse team is picking fresh farm produce.' : 'Ozone washed & sealed in eco packaging.', currentStatus, ['PICKING', 'PACKING', 'READY_FOR_HANDOVER', 'READY_FOR_PICKUP', 'HANDED_TO_DELIVERY_BOY', 'ASSIGNED', 'DELIVERY_BOY_ACCEPTED', 'ACCEPTED', 'PICKED_UP', 'OUT_FOR_DELIVERY', 'ARRIVED', 'CUSTOMER_VERIFIED', 'DELIVERED'])}
        ${renderLifecycleStep('OUT_FOR_DELIVERY', 'Out for Delivery in EV', hasRider ? `EV Pilot ${riderName} is en route.` : (currentStatus === 'READY_FOR_HANDOVER' || currentStatus === 'HANDED_TO_DELIVERY_BOY' || currentStatus === 'DELIVERY_BOY_ACCEPTED') ? 'Order handed over to courier pilot.' : 'Dispatched from Dark Store.', currentStatus, ['HANDED_TO_DELIVERY_BOY', 'ASSIGNED', 'DELIVERY_BOY_ACCEPTED', 'ACCEPTED', 'PICKED_UP', 'OUT_FOR_DELIVERY', 'ARRIVED', 'CUSTOMER_VERIFIED', 'DELIVERED'])}
        ${renderLifecycleStep('ARRIVED', 'Arrived at Doorstep', currentStatus === 'CUSTOMER_VERIFIED' ? '✓ Customer verified with OTP.' : 'EV Pilot arrived. Provide your 4-digit OTP.', currentStatus, ['ARRIVED', 'CUSTOMER_VERIFIED', 'DELIVERED'])}
      </div>
      ...
      ${hasRider ? `
        ...
        <div class="p-3.5 rounded-2xl bg-stone-50 border border-stone-200/80 flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center font-bold text-lg">🛵</div>
            <div>
              <h5 class="font-bold text-xs sm:text-sm text-emerald-950">${riderName}</h5>
              <p class="text-[10px] text-emerald-700 font-medium">★ 4.9 • EV Delivery Partner</p>
            </div>
          </div>
          <div class="flex gap-2">
            <a href="tel:${riderPhone}" class="px-3 py-1.5 rounded-xl bg-emerald-800 text-white text-xs font-bold hover:bg-emerald-900 transition-colors">
              📞 Call
            </a>
          </div>
        </div>
...
function renderLifecycleStep(stepKey, title, desc, currentStatus, activeInStatuses) {
  const normCurrent = (currentStatus || '').toUpperCase();
  const isCompleted = activeInStatuses.slice(1).includes(normCurrent);
  const isActive = normCurrent === stepKey || 
    (stepKey === 'ORDER_RECEIVED' && (normCurrent === 'ORDER_RECEIVED' || normCurrent === 'ORDER_PLACED')) ||
    (stepKey === 'ORDER_CONFIRMED' && (normCurrent === 'ORDER_CONFIRMED' || normCurrent === 'CONFIRMED')) ||
    (stepKey === 'PACKING' && ['PICKING', 'PACKING', 'READY_FOR_HANDOVER', 'READY_FOR_PICKUP'].includes(normCurrent)) ||
    (stepKey === 'OUT_FOR_DELIVERY' && ['HANDED_TO_DELIVERY_BOY', 'ASSIGNED', 'DELIVERY_BOY_ACCEPTED', 'ACCEPTED', 'PICKED_UP', 'OUT_FOR_DELIVERY'].includes(normCurrent)) ||
    (stepKey === 'ARRIVED' && ['ARRIVED', 'CUSTOMER_VERIFIED'].includes(normCurrent));

  return `
    <div class="flex items-start gap-4 relative z-10">
      <div class="w-8 h-8 rounded-full ${isCompleted ? 'bg-emerald-600 text-white' : isActive ? 'bg-emerald-500 text-white shadow-md animate-pulse ring-4 ring-emerald-100' : 'bg-stone-200 text-stone-500'} flex items-center justify-center text-xs font-bold shadow-xs shrink-0">
        ${isCompleted ? '✓' : isActive ? '⚡' : '•'}
      </div>
      <div>
        <div class="flex items-center gap-2">
          <h4 class="font-bold text-xs sm:text-sm ${isActive ? 'text-emerald-800 font-extrabold' : isCompleted ? 'text-emerald-950' : 'text-stone-500'}">${title}</h4>
          ${isActive ? '<span class="text-[10px] bg-emerald-100 text-emerald-900 font-bold px-1.5 rounded">Active</span>' : ''}
        </div>
        <p class="text-[11px] ${isActive ? 'text-stone-700 font-medium' : 'text-stone-500'}">${desc}</p>
      </div>
    </div>
  `;
}
...
function getProgressWidth(status) {
  ...
  switch (s) {
    case 'ORDER_RECEIVED':
    case 'ORDER_PLACED': return 'w-1/12';
    case 'ORDER_CONFIRMED':
    case 'CONFIRMED': return 'w-2/12';
    case 'PICKING': return 'w-3/12';
    case 'PACKING': return 'w-4/12';
    case 'READY_FOR_HANDOVER':
    case 'READY_FOR_PICKUP': return 'w-5/12';
    case 'HANDED_TO_DELIVERY_BOY':
    case 'ASSIGNED': return 'w-6/12';
    case 'DELIVERY_BOY_ACCEPTED':
    case 'ACCEPTED': return 'w-7/12';
    case 'PICKED_UP': return 'w-8/12';
    case 'OUT_FOR_DELIVERY': return 'w-10/12';
    case 'ARRIVED': return 'w-11/12';
...
function renderTodaysDeals() {
  ...
  const allProds = getAllCatalogProducts().filter(p => 
    (p.status === 'ACTIVE' || p.status === 'LOW_STOCK' || !p.status) &&
    p.status !== 'SUSPENDED' && p.status !== 'INACTIVE' && p.status !== 'DRAFT' && p.status !== 'DELETED' && p.status !== 'OUT_OF_STOCK' &&
    (p.stock === undefined || p.stock > 0 || (p.stockCount !== undefined && p.stockCount > 0)) &&
    !isProductSuspended(p.id)
  );

  let dealsItems = allProds.filter(p => {
    const w = p.weights && p.weights[p.selectedWeightIndex || 0];
    return w && (w.originalPrice > w.price || (w.discount && String(w.discount).includes('%')));
  });
  if (dealsItems.length < 6) {
    dealsItems = allProds.slice(0, 6);
  } else {
    dealsItems = dealsItems.slice(0, 8);
  }
...
function renderHomeProductGrid() {
  ...
  if (homeActiveCategory === 'vegetables') {
    ...
    homeItems = allProds.filter(p => (p.categories && p.categories.includes('leafy')) || (p.name && /palak|methi|spinach|coriander|mint|lettuce|green/i.test(p.name)));
...
function updateCategoryCounts() {
  ...
  const dailyCount = activeCombos.filter(c => /daily|essential/i.test(c.name)).length || 
                     activeVeg.filter(v => (v.categories && (v.categories.includes('daily') || v.categories.includes('staples'))) || (v.name && /daily essentials|potato|tomato|onion/i.test(v.name))).length || 1;
...
window.setCategory = function(cat) {
  ...
  document.querySelectorAll('.veg-nav-cat-btn').forEach(btn => {
    const isCombo = btn.dataset.category === 'combos' || btn.dataset.category === 'daily';
...
function applyFiltersAndRender() {
  ...
  if (filterState.category !== 'all') {
    if (filterState.category === 'combos' || filterState.category === 'daily') {
      const comboSec = document.getElementById('combos-section');
      if (comboSec) comboSec.scrollIntoView({ behavior: 'smooth' });
      if (filterState.category === 'combos') return;
      filtered = filtered.filter(v => 
        (v.categories && (v.categories.includes('daily') || v.categories.includes('staples'))) ||
        (v.subcategory && /daily|combo|staple/i.test(v.subcategory)) ||
        (v.name && /essential|daily|basket|potato|tomato|onion/i.test(v.name))
      );
    } else {
      filtered = filtered.filter(v => v.categories && v.categories.includes(filterState.category));
    }
  }

  filtered = filtered.filter(v => {
    const currentPrice = (v.weights && v.weights[v.selectedWeightIndex || 0]) ? v.weights[v.selectedWeightIndex || 0].price : (v.price || 0);
    return filterState.maxPrice ? currentPrice <= filterState.maxPrice : true;
  });
...
function applyFruitFiltersAndRender() {
  ...
  let filtered = allFruitsData.filter(v => 
  ...
  if (fruitFilterState.category !== 'all') {
    filtered = filtered.filter(v => v.categories && v.categories.includes(fruitFilterState.category));
  }
...
function applyGroceryFiltersAndRender() {
  ...
  let filtered = allGroceryData.filter(v => 
    (v.status === 'ACTIVE' || v.status === 'LOW_STOCK' || !v.status) &&
    v.status !== 'SUSPENDED' && v.status !== 'INACTIVE' && v.status !== 'DRAFT' && v.status !== 'DELETED' && v.status !== 'OUT_OF_STOCK' &&
    (v.stock === undefined || v.stock > 0 || (v.stockCount !== undefined && v.stockCount > 0)) &&
    !isProductSuspended(v.id)
  ...
  if (groceryFilterState.category !== 'all') {
    filtered = filtered.filter(v => v.categories && v.categories.includes(groceryFilterState.category));
  }
...
function getOffersCatalog() {
  ...
  const discounted = allProds.filter(p => {
    const w = p.weights && p.weights[p.selectedWeightIndex || 0];
    return w && (w.originalPrice > w.price || (w.discount && String(w.discount).includes('%')));
  });

  return discounted.length > 0 ? discounted : allProds;
}
...
window.clearCart = function(confirmPrompt = true) {
  const count = Object.keys(cart || {}).length;
  if (count === 0) {
    showToast('Your basket is already empty.', 'info');
    return;
  }

  if (confirmPrompt && !confirm('Are you sure you want to remove all items from your basket?')) {
    return;
  }
  ...
  appliedCoupon = null;
  saveCartState(cart);
  saveStoredState('sabjihub_coupon', null);
  try {
    localStorage.removeItem('sabjihub_coupon');
  } catch (e) {}
