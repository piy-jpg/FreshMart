  ...
  return {
    ...
    products: [
      ...
      },
      {
            ...
            "lowStockLimit": 10,
            "reorderLevel": 20,
            "farmer": "SabjiHub Direct Farm Cooperative",
            "hubId": "hub_blr_indiranagar",
            "status": "ACTIVE",
            "expressEligible": true,
            "harvestDate": "2026-09-12",
...
class Database {
  ...
  logActivity(user, action, entity, entityId, details) {
    const operatorEmail = typeof user === 'string' ? user : (user.email || user.name || 'Owner');
    const logItem = {
  ...
  resetRateLimit(key) {
    if (this.rateLimits) this.rateLimits.delete(key);
  }
  ...
  createStaffUser(data) {
    ...
    const { hash, salt } = data.password ? this.hashPassword(data.password) : { hash: '', salt: '' };
    const now = new Date().toISOString();
    const assignedRole = data.role || 'SUPPORT_AGENT';
    ...
    const staffUser = {
      ...
      status: 'ACTIVE',
      emailVerified: true,
      membership: `${assignedRole.replace('_', ' ')} Team`,
      passwordHash: hash,
      passwordSalt: salt,
      provider: 'local',
      active: true,
  ...
  checkDatabaseIntegrity() {
    ...
    const validRoles = ['OWNER', 'ADMIN', 'INVENTORY_MANAGER', 'ORDER_MANAGER', 'HUB_MANAGER', 'PROCUREMENT_MANAGER', 'DELIVERY_MANAGER', 'DELIVERY_PARTNER', 'DISPATCHER', 'SUPPORT_AGENT', 'CUSTOMER'];
