...
function requireOwner(req, res) {
  ...
  const role = freshUser ? freshUser.role : user.role;
  const email = freshUser ? freshUser.email : user.email;
...
const server = http.createServer(async (req, res) => {
  ...
  if (pathname.startsWith('/api/')) {
    ...
    if (pathname === '/api/auth/login' && method === 'POST') {
      ...
      // Rate limiting: 5 failed attempts per IP / per email in 15 mins
      const ipRate = db.checkRateLimit('login_ip:' + ip, 10, 15 * 60 * 1000);
      const emailRate = db.checkRateLimit('login_email:' + email, 5, 15 * 60 * 1000);

      if (!ipRate.allowed || !emailRate.allowed) {
        const retrySec = Math.max(ipRate.retryAfterSeconds || 0, emailRate.retryAfterSeconds || 0);
      ...
      if (user.status === 'BLOCKED' || (user.lockUntil && new Date(user.lockUntil) > new Date())) {
        return sendJson(res, 403, { error: 'This account has been temporarily locked. Please contact support.' });
      }
      ...
      db.resetRateLimit('login_email:' + email);
      ...
      db.logActivity(user.name, user.role === 'OWNER' ? 'OWNER_LOGIN' : 'CUSTOMER_LOGIN', 'Users', user.id, 'Session created');
    ...
    if (pathname === '/api/orders' && method === 'GET') {
      ...
      if (riderId) filtered = filtered.filter(o => o.deliveryPartnerId === riderId);
    ...
    if (pathname === '/api/orders' && method === 'POST') {
      ...
      const newOrder = {
        ...
        deliveryPartnerRating: rider.rating,
        ...
        timeline: [
          {
            ...
            desc: `Payment confirmed via ${body.paymentMethod || 'UPI'}. Routing to ${hub.name}.`,
    ...
    if (pathname === '/api/delivery/available-orders' && method === 'GET') {
      const orders = db.getAll('orders');
      const available = orders.filter(o => ['READY_FOR_PICKUP', 'PACKED', 'CONFIRMED'].includes(o.orderStatus));
    ...
    if (riderAcceptMatch && method === 'PATCH') {
      const order = db.getById('orders', riderAcceptMatch[1]);
      if (!order) return sendJson(res, 404, { error: 'Order not found' });

      order.timeline.push({
        status: 'GOING_TO_HUB',
        title: 'Delivery Partner Assigned',
        desc: `${order.deliveryPartnerName} is navigating to ${order.hubName}.`,
        time: new Date().toISOString()
      });
      db.update('orders', order.id, order);
      broadcastEvent('ORDER_UPDATED', order);
    ...
    if (riderPickupMatch && method === 'PATCH') {
      const order = db.getById('orders', riderPickupMatch[1]);
      if (!order) return sendJson(res, 404, { error: 'Order not found' });
      ...
      order.timeline.push({
        ...
        desc: `Package received by ${order.deliveryPartnerName} from ${order.hubName}.`,
        time: new Date().toISOString()
      });
      db.update('orders', order.id, order);
      broadcastEvent('ORDER_UPDATED', order);
    ...
    if (riderOutMatch && method === 'PATCH') {
      const order = db.getById('orders', riderOutMatch[1]);
      if (!order) return sendJson(res, 404, { error: 'Order not found' });

      order.orderStatus = 'OUT_FOR_DELIVERY';
      order.status = 'OUT_FOR_DELIVERY';
      order.estimatedDeliveryTime = '18 Minutes';
      order.timeline.push({
        status: 'OUT_FOR_DELIVERY',
        title: 'Out for Delivery',
        desc: `${order.deliveryPartnerName} is en route via EV. Live tracking active.`,
        time: new Date().toISOString()
      });
      db.update('orders', order.id, order);
      broadcastEvent('ORDER_UPDATED', order);
    ...
    if (riderArrivedMatch && method === 'PATCH') {
      const order = db.getById('orders', riderArrivedMatch[1]);
      if (!order) return sendJson(res, 404, { error: 'Order not found' });

      order.orderStatus = 'ARRIVED';
      order.status = 'ARRIVED';
      order.estimatedDeliveryTime = 'At your doorstep';
      order.timeline.push({
        status: 'ARRIVED',
        title: 'Driver Arrived at Doorstep',
        desc: `${order.deliveryPartnerName} has arrived at your address. Ready for OTP verification.`,
        time: new Date().toISOString()
      });
      db.update('orders', order.id, order);
      broadcastEvent('ORDER_UPDATED', order);
    ...
    if (riderVerifyOtpMatch && method === 'POST') {
      ...
      const order = db.getById('orders', riderVerifyOtpMatch[1]);
      if (!order) return sendJson(res, 404, { error: 'Order not found' });

      if (inputOtp !== String(order.deliveryOtp)) {
        return sendJson(res, 400, {
      ...
      order.timeline.push({
        ...
        desc: `Verified with OTP ${order.deliveryOtp}. Handover complete by ${order.deliveryPartnerName}.`,
        time: new Date().toISOString()
      });
      ...
      db.insert('notifications', {
        ...
        message: 'Your farm-fresh basket has been delivered. Enjoy the fresh harvest!',
        type: 'DELIVERY',
        read: false,
        createdAt: new Date().toISOString()
      });

      db.logActivity(order.deliveryPartnerName, 'ORDER_DELIVERED', 'Order', order.orderId, `Order #${order.orderId} delivered and verified with OTP.`);
      broadcastEvent('ORDER_UPDATED', order);
      return sendJson(res, 200, { success: true, message: 'OTP verified. Order marked as DELIVERED.', order });
    }
    ...
    // =========================================================================
    // COMPLETE EXECUTIVE OWNER MANAGEMENT SUITE (/api/owner/*)
    // =========================================================================
    ...
    if (pathname.startsWith('/api/owner/')) {
      ...
      if (pathname.startsWith('/api/owner/orders/') && method === 'PATCH') {
        const orderId = pathname.replace(/^\/api\/owner\/orders\//, '').replace(/\/status\/?$/, '');
        const orders = db.getAll('orders') || [];
        const order = orders.find(o => o.id === orderId || o.orderId === orderId);
        if (!order) return sendJson(res, 404, { error: 'Order not found' });

        const body = await parseBody(req);
        const oldStatus = order.orderStatus || order.status || 'CONFIRMED';
        ...
        if (body.status) {
          ...
          order.timeline.push({
            ...
            title: `Order marked as ${body.status}`,
            desc: body.notes || 'Updated via Executive Owner Dashboard'
          });
        ...
        if (body.hubId) {
          ...
          if (hub) {
            ...
            order.timeline.push({
              ...
              time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              title: `Hub Reassigned to ${hub.name}`,
              desc: `Reassigned by Owner`
            });
          }
        }

        if (body.deliveryPartnerId) {
          const rider = db.getById('delivery_partners', body.deliveryPartnerId);
          if (rider) {
            ...
            order.timeline.push({
              status: order.orderStatus,
              time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
              title: `Rider Assigned: ${rider.name}`,
              desc: `Vehicle: ${rider.vehicle || 'EV Bike'}, Phone: ${rider.phone}`
            });
          }
        }
      ...
      if (pathname === '/api/owner/inventory' && method === 'GET') {
        const ledger = db.getInventoryLedger();
      ...
      if (pathname === '/api/owner/inventory/adjust' && method === 'POST') {
        ...
        db.insert('inventory_movements', {
          ...
          before: oldStock,
          after: prod.stock,
          reason: reason || 'Owner Manual Stock Adjustment',
          user: owner.name + ' (Owner)'
        });
      ...
      // 5. Staff & Sub-Admin Management (Owner Exclusive)
      ...
      if (pathname === '/api/owner/staff' && method === 'POST') {
        ...
        try {
          ...
          db.logActivity(owner.name, 'STAFF_CREATED', 'Staff', staffUser.id, `Created staff member ${staffUser.name} (${staffUser.email}) with role ${staffUser.role}`);
      ...
      if (pathname.startsWith('/api/owner/staff/') && method === 'PATCH') {
        ...
        try {
          ...
          } else if (body.status === 'DEACTIVATED') {
  ...
  if (pathname === '/owner.html') {
    ...
    if (role !== 'OWNER' && !isOwnerEmail(email)) {
      ...
      return res.end('<!DOCTYPE html><html><body style="font-family:sans-serif;text-align:center;padding:60px 20px;background:#fdfcf7;color:#1c1917;"><div style="max-width:520px;margin:0 auto;background:#fff;padding:40px;border-radius:24px;box-shadow:0 10px 30px rgba(0,0,0,0.06);border:1px solid #e7e5e4;"><div style="font-size:48px;margin-bottom:12px;">👑</div><h1 style="color:#991b1b;font-size:24px;margin:0 0 8px;">403 Forbidden</h1><p style="font-size:14px;color:#57534e;line-height:1.6;">Owner access required. You do not have executive owner privileges to enter this dashboard.</p><div style="margin-top:24px;"><a href="/" style="display:inline-block;padding:12px 24px;background:#047857;color:#fff;text-decoration:none;border-radius:14px;font-weight:bold;font-size:13px;">← Return to FreshMart Storefront</a></div></div></body></html>');
    }
  }
